                  ...
                  <c:if test="${ totaldata > 12 }">
						<div class="col-lg-12 d-flex justify-content-center">
                    		<div class="pagination">
                      			<a href="<c:url value="/category${ catid }/p1" />">&laquo;</a>
                      			
                      		<c:forEach var="item" begin="1" end="${ paginationinfo.total }" varStatus="itemS">
                      			<c:if test="${ itemS.index == paginationinfo.current }">
                      				<a href="<c:url value="/category${ catid }/p${ itemS.index }" />" class="active">${ itemS.index }</a>
                      			</c:if>
                      			<c:if test="${ itemS.index != paginationinfo.current }">
                      				<a href="<c:url value="/category${ catid }/p${ itemS.index }" />">${ itemS.index }</a>
                      			</c:if>
                      			<c:set var="lastindex" value="${ itemS.index }" />
                      		</c:forEach>

			                    <a href="<c:url value="/category${ catid }/p${ lastindex }" />">&raquo;</a>
                    		</div>
                  		</div>
                	</c:if>
